    ...
    if (onlyBorder) {
      const mouseMove = ({ clientX, clientY }: MouseEvent): void => {
        if (element.isConnected) {
          ...
          element.style.borderImage = `radial-gradient(50px at ${clientX - x}px ${clientY - y}px, rgba(${hovered ? "255, 255, 255, 80%" : "200, 200, 200, 60%"}), transparent) 1 / ${border}px / 0 stretch`;
        } else {
      ...
      const mouseMove = ({ offsetX: x, offsetY: y }: MouseEvent): void => {
        ...
        element.style.borderImage = `radial-gradient(50px at ${x}px ${y}px, rgba(200, 200, 200, 60%), transparent) 1 / ${border}px / 0 stretch`;
      };
